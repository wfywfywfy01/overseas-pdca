    ...
    for attempt in range(retries):
        try:
            ...
            if data.get("code") == 200:
                return data
            print(f"[API] non-200 code from {endpoint}: {data.get('message')} (payload={payload})")
            return data
        except Exception as e:
            if attempt == retries - 1:
                print(f"[API] failed {endpoint} after {retries} attempts: {e}")
                raise
            time.sleep(1)
    return {}


def api_post_paged(endpoint: str, base_payload: dict, page_size: int = 100,
                   info_filters: dict | None = None) -> list:
...
def append_memory(agent_name: str, section: str, content: str):
    ...
    with open(path, "a", encoding="utf-8") as f:
        f.write(entry)
    print(f"[MEMORY] {agent_name}: appended '{section}'")


def bump_memory_version(agent_name: str):
